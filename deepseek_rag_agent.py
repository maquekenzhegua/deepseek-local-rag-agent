...
# Monkey-patch ollama's httpx client to use requests instead (httpx broken on this machine)
import requests as _requests
from ollama._types import ResponseError as _ResponseError
import ollama._client as _ollama_client

_patched_client_cls = type(_ollama_client)

def _patched_request_raw(self, *args, **kwargs):
    """Replace httpx with requests."""
    method, endpoint = args[0], args[1]
    url = str(self._client.base_url) + endpoint
    resp = _requests.request(method, url, json=kwargs.get('json'), stream=kwargs.get('stream', False))
    if resp.status_code >= 400:
        raise _ResponseError(resp.text, resp.status_code)
    return resp

_patched_client_cls._request_raw = _patched_request_raw
...
class OllamaEmbedderr(Embeddings):
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        for i, text in enumerate(texts):
            try:
                resp = ollama.embeddings(model=self.model_name, prompt=text[:500])
